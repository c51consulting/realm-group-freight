/**
 * @fileoverview Sequelize models and associations for REALM Ag Marketplace.
 *
 * Models:
 *   User, Listing, FeedTest, Offer, Order,
 *   FreightJob, WeighbridgeEvent, ProofOfDelivery, Review, Message
 */
...
const Listing = sequelize.define('Listing', {
  ...
  status:                 { type: DataTypes.ENUM('draft', 'active', 'paused', 'sold', 'expired', 'cancelled'), defaultValue: 'draft' },
  ...
  pricePerUnit:           { type: DataTypes.DECIMAL(12, 2) },
  pricePerTonneEquiv:     { type: DataTypes.DECIMAL(12, 2) },
  quantityAvailable:      { type: DataTypes.DECIMAL(12, 2) },
  quantityUnit:           { type: DataTypes.STRING },
  minimumOrder:           { type: DataTypes.DECIMAL(12, 2) },
...
const Offer = sequelize.define('Offer', {
  ...
  totalPrice:      { type: DataTypes.DECIMAL(14, 2) },
  freightIncluded: { type: DataTypes.BOOLEAN, defaultValue: false },
  freightPrice:    { type: DataTypes.DECIMAL(12, 2) },
...
const Order = sequelize.define('Order', {
  ...
  freightAmount:          { type: DataTypes.DECIMAL(12, 2) },
...
// ─── FREIGHT JOB MODEL ───────────────────────────────────────────────────────
const FreightJob = sequelize.define('FreightJob', {
  id:              { type: DataTypes.UUID,   defaultValue: DataTypes.UUIDV4, primaryKey: true },
  status:          { type: DataTypes.ENUM('open', 'quoted', 'assigned', 'in_transit', 'delivered', 'cancelled'), defaultValue: 'open' },
  pickupAddress:   { type: DataTypes.JSONB,  allowNull: false },
  pickupLat:       { type: DataTypes.FLOAT },
  pickupLng:       { type: DataTypes.FLOAT },
  deliveryAddress: { type: DataTypes.JSONB,  allowNull: false },
  deliveryLat:     { type: DataTypes.FLOAT },
  deliveryLng:     { type: DataTypes.FLOAT },
  materialType:    { type: DataTypes.ENUM('hay', 'straw', 'silage', 'grain', 'seed', 'pellets', 'fertiliser', 'supplement', 'drums', 'bulk_liquid', 'other') },
  materialDesc:    { type: DataTypes.TEXT },
  weightKg:        { type: DataTypes.DECIMAL(12, 2) },
  volumeM3:        { type: DataTypes.DECIMAL(10, 2) },
  requiredBy:      { type: DataTypes.DATE },
  priceOffered:    { type: DataTypes.DECIMAL(12, 2) },
  priceAgreed:     { type: DataTypes.DECIMAL(12, 2) },
...
const WeighbridgeEvent = sequelize.define('WeighbridgeEvent', {
  id:               { type: DataTypes.UUID,   defaultValue: DataTypes.UUIDV4, primaryKey: true },
  source:           { type: DataTypes.ENUM('api', 'csv_import', 'ocr_upload', 'manual'), allowNull: false },
  ...
  tareWeight:       { type: DataTypes.DECIMAL(12, 2) },
  netWeight:        { type: DataTypes.DECIMAL(12, 2) },
  ...
  freightJobId:     { type: DataTypes.UUID },
});

// ─── PROOF OF DELIVERY MODEL ──────────────────────────────────────────────────
const ProofOfDelivery = sequelize.define('ProofOfDelivery', {
  id:              { type: DataTypes.UUID,   defaultValue: DataTypes.UUIDV4, primaryKey: true },
  status:          { type: DataTypes.ENUM('pending', 'submitted', 'accepted', 'rejected'), defaultValue: 'pending' },
  photoUrls:       { type: DataTypes.JSONB,  defaultValue: [] },
  signatureUrl:    { type: DataTypes.STRING },
  notes:           { type: DataTypes.TEXT },
  gpsLat:          { type: DataTypes.FLOAT },
  gpsLng:          { type: DataTypes.FLOAT },
  weighEventId:    { type: DataTypes.UUID },
...
User.hasMany(FeedTest, { as: 'uploadedFeedTests', foreignKey: 'uploadedBy' });
FeedTest.belongsTo(User, { as: 'uploader', foreignKey: 'uploadedBy' });
...
// Order ↔ FreightJob
Order.hasMany(FreightJob, { as: 'freightJobs', foreignKey: 'orderId' });
FreightJob.belongsTo(Order, { foreignKey: 'orderId' });
User.hasMany(FreightJob, { as: 'postedFreightJobs', foreignKey: 'postedBy' });
FreightJob.belongsTo(User, { as: 'poster',  foreignKey: 'postedBy' });
FreightJob.belongsTo(User, { as: 'carrier', foreignKey: 'carrierId' });

// Order ↔ WeighbridgeEvent
Order.hasMany(WeighbridgeEvent, { as: 'weighEvents', foreignKey: 'orderId' });
WeighbridgeEvent.belongsTo(Order, { foreignKey: 'orderId' });
FreightJob.hasMany(WeighbridgeEvent, { as: 'weighEvents', foreignKey: 'freightJobId' });
WeighbridgeEvent.belongsTo(FreightJob, { foreignKey: 'freightJobId' });

// Order ↔ ProofOfDelivery
Order.hasMany(ProofOfDelivery, { as: 'proofOfDeliveries', foreignKey: 'orderId' });
ProofOfDelivery.belongsTo(Order, { foreignKey: 'orderId' });
User.hasMany(ProofOfDelivery, { as: 'submittedPODs', foreignKey: 'submittedBy' });
ProofOfDelivery.belongsTo(User, { as: 'submitter', foreignKey: 'submittedBy' });
